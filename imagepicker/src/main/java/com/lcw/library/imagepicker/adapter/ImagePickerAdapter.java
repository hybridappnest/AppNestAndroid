...
public class ImagePickerAdapter extends RecyclerView.Adapter<ImagePickerAdapter.BaseHolder> {
    ...
    public ImagePickerAdapter(Context context, List<MediaFile> mediaFiles,
                              PickerViewModel mPickerViewModel) {
        ...
        this.mPickerViewModel = mPickerViewModel;
    }
    ...
    public BaseHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType == ItemType.ITEM_TYPE_CAMERA) {
            ...
            TextView tvCamera = view.findViewById(R.id.tv_title);
            if (isShowVideo) {
                tvCamera.setText(R.string.take_photo_video);
            } else {
    ...
    private void bindMedia(MediaHolder mediaHolder, MediaFile mediaFile) {
        ...
        if (!TextUtils.isEmpty(imagePath)) {
            ...
            if (mPickerViewModel.isImageSelect(imagePath)) {
                mediaHolder.mImageView.setColorFilter(Color.parseColor("#77000000"));
                mediaHolder.mImageCheck.setImageDrawable(mContext.getResources().getDrawable(R.drawable.icon_image_checked));
            } else {
                mediaHolder.mImageView.setColorFilter(null);
                mediaHolder.mImageCheck.setImageDrawable(mContext.getResources().getDrawable(R.drawable.shape_icon_image_check));
            }

            try {
                ConfigManager.getInstance().getImageLoader().loadImage(mediaHolder.mImageView,
                        imagePath);
            } catch (Exception e) {
                e.printStackTrace();
            }
