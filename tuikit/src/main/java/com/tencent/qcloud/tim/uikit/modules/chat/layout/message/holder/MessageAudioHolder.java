...
//        initVoiceText("翻译信息");
    ...
    private void checkUnReadAudio(final MessageInfo msg){
        ...
        for(int i = index;i<messageInfos.size();i++){
            ...
            if(messageInfo.getCustomInt() == UNREAD&&messageInfo.getExtra().equals("[语音消息]")&&!messageInfo.isSelf()){
                try {
                    ...
                    if(view1 != null && recyclerView.getChildViewHolder(view1) instanceof MessageAudioHolder){
                        MessageAudioHolder myViewHolder = (MessageAudioHolder) recyclerView.getChildViewHolder(view1);//获取到对应Item的ViewHolder
