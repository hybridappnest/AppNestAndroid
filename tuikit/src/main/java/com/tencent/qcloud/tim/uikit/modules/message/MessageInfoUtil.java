...
public class MessageInfoUtil {
    ...
    public static MessageInfo buildCustomFaceMessage(int groupId, String faceName) {
        MessageInfo info = new MessageInfo();
        V2TIMMessage v2TIMMessage = V2TIMManager.getMessageManager().createFaceMessage(groupId,
                faceName.getBytes());
     ...
     * @return
     */
    public static MessageInfo buildImageMessage1(final String path, boolean compressed) {
        final MessageInfo info = new MessageInfo();
        V2TIMMessage v2TIMMessage = V2TIMManager.getMessageManager().createImageMessage(path);
    ...
    public static MessageInfo buildVideoMessage(String imgPath, String videoPath, int width,
        ...
        MessageInfo info = new MessageInfo();
        V2TIMMessage v2TIMMessage = V2TIMManager.getMessageManager().createVideoMessage(videoPath
                , "mp4", (int) duration / 1000, imgPath);
    ...
    public static MessageInfo buildAudioMessage(String recordPath, int duration,
        ...
        V2TIMMessage v2TIMMessage =
                V2TIMManager.getMessageManager().createSoundMessage(recordPath, duration / 1000);
    ...
    public static MessageInfo buildFileMessage(Uri fileUri) {
        ...
        if (file.exists()) {
            MessageInfo info = new MessageInfo();
            V2TIMMessage v2TIMMessage =
                    V2TIMManager.getMessageManager().createFileMessage(filePath, file.getName());
     ...
     * @return
     */
    public static MessageInfo buildCustomAudioMessage(String data) {
        MessageInfo info = new MessageInfo();
        V2TIMMessage v2TIMMessage =
                V2TIMManager.getMessageManager().createCustomMessage(data.getBytes());
        info.setSelf(true);
        info.setTimMessage(v2TIMMessage);
        info.setMsgTime(System.currentTimeMillis() / 1000);
        info.setMsgType(MessageInfo.MSG_TYPE_AUDIO_CUSTOM);
        info.setFromUser(V2TIMManager.getInstance().getLoginUser());
        info.setExtra("[语音消息]");
        return info;
    }
    ...
    public static MessageInfo buildCustomMessage(String data) {
        ...
        try {
            ...
            if (jsonObject.containsKey("title")) {
                pushDesc = jsonObject.getString("title");
            }
            if (pushDesc == null || !pushDesc.isEmpty()) {
                info.setExtra(pushDesc);
            } else {
                info.setExtra("[自定义消息]");
            }
        } catch (Exception e) {
    ...
    public static V2TIMMessage buildGroupCustomMessage(String customMessage) {
        V2TIMMessage v2TIMMessage =
                V2TIMManager.getMessageManager().createCustomMessage(customMessage.getBytes());
        return v2TIMMessage;
    }
     ...
     * @return
     */
    public static List<MessageInfo> TIMMessages2MessageInfos(List<V2TIMMessage> timMessages,
                                                             boolean isGroup) {
        if (timMessages == null) {
            return null;
        }
    ...
    public static MessageInfo createMessageInfo(V2TIMMessage timMessage) {
        ...
        if (type == V2TIMMessage.V2TIM_ELEM_TYPE_CUSTOM) {
            ...
            if (data.equals(MessageCustom.BUSINESS_ID_GROUP_CREATE)) {
                ...
                try {
                    ...
                    if (!TextUtils.isEmpty(messageCustom.businessID) && messageCustom.businessID.equals(MessageCustom.BUSINESS_ID_GROUP_CREATE)) {
                        msgInfo.setMsgType(MessageInfo.MSG_TYPE_GROUP_CREATE);
                        String message =
                                TUIKitConstants.covert2HTMLString(messageCustom.opUser) + messageCustom.content;
                        msgInfo.setExtra(message);
                    } else if (!TextUtils.isEmpty(messageCustom.event_id)) {
                        msgInfo.setMsgType(MessageInfo.MSG_TYPE_CUSTOM_BAOJING);
                        msgInfo.setExtra(messageCustom);
                    } else if (!TextUtils.isEmpty(messageCustom.getLocalPath())) {
                        if (!msgInfo.isSelf() && TextUtils.isEmpty(messageCustom.getRemoteUrl())) {
                            return null;
                        }
                        msgInfo.setMsgType(MessageInfo.MSG_TYPE_AUDIO_CUSTOM);
                        if (msgInfo.isSelf() && new File(messageCustom.getLocalPath()).exists()) {
                            ...
                            if (!file.exists()) {
                                DownloadUtils downloadUtils =
                                        new DownloadUtils(TUIKit.getAppContext());
                                downloadUtils.downloadFile(messageCustom.getRemoteUrl(), path,
                                        name, new DownloadListener() {
                                            @Override
                                            public void onStart() {
                                                Log.e(TAG, "onStart: ");
                                            }
                        ...
                        if (callModel != null) {
                            ...
                            switch (callModel.action) {
                                case CallModel.VIDEO_CALL_ACTION_DIALING:
                                    content = isGroup ? ("\"" + senderShowName + "\"" + "发起群通话")
                                            : ("发起通话");
                                    break;
                                ...
                                case CallModel.VIDEO_CALL_ACTION_LINE_BUSY:
                                    content = isGroup ? ("\"" + senderShowName + "\"" + "忙线") :
                                            "对方忙线";
                                    break;
                                case CallModel.VIDEO_CALL_ACTION_REJECT:
                                    content = isGroup ? ("\"" + senderShowName + "\"" + "拒绝群通话")
                                            : "拒绝通话";
                                    break;
                                case CallModel.VIDEO_CALL_ACTION_SPONSOR_TIMEOUT:
                                    if (isGroup && callModel.invitedList != null && callModel.invitedList.size() == 1
                                        ...
                                        StringBuilder inviteeShowStringBuilder =
                                                new StringBuilder();
                                        ...
                                        content = isGroup ?
                                                ("\"" + inviteeShowStringBuilder.toString() +
                                                        "\"" + "无应答") : "无应答";
                                    }
                                    break;
                                case CallModel.VIDEO_CALL_ACTION_ACCEPT:
                                    content = isGroup ? ("\"" + senderShowName + "\"" + "已接听") :
                                            "已接听";
                                    break;
                                case CallModel.VIDEO_CALL_ACTION_HANGUP:
                                    content = isGroup ? "结束群通话" :
                                            "结束通话，通话时长：" + DateTimeUtil.formatSecondsTo00(callModel.duration);
                                    break;
                                default:
                                    content = "不能识别的通话指令";
                                    break;
            ...
            if (tipsType == V2TIMGroupTipsElem.V2TIM_GROUP_TIPS_TYPE_MEMBER_INFO_CHANGE) {
                List<V2TIMGroupMemberChangeInfo> modifyList =
                        groupTipElem.getMemberChangeInfoList();
            ...
            if (type == V2TIMMessage.V2TIM_ELEM_TYPE_TEXT) {
                ...
                if (msgInfo.isSelf() && new File(soundElemEle.getPath()).exists()) {
                    msgInfo.setDataPath(soundElemEle.getPath());
                } else {
                    final String path =
                            TUIKitConstants.RECORD_DOWNLOAD_DIR + soundElemEle.getUUID();
                    ...
                    if (!file.exists()) {
                        soundElemEle.downloadSound(path, new V2TIMDownloadCallback() {
                            ...
                            public void onProgress(V2TIMElem.V2ProgressInfo progressInfo) {
                                ...
                                TUIKitLog.i("MessageInfoUtil getSoundToFile",
                                        "progress:" + progress);
                            }
    ...
    public static int TIMElemType2MessageInfoType(int type) {
        switch (type) {
