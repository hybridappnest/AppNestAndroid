...
public class GroupInfoProvider {
        ...
        V2TIMManager.getGroupManager().getGroupMemberList(mGroupInfo.getId(), V2TIMGroupMemberFullInfo.V2TIM_GROUP_MEMBER_FILTER_ALL, nextSeq, new V2TIMValueCallback<V2TIMGroupMemberInfoResult>() {
            ...
            public void onSuccess(V2TIMGroupMemberInfoResult v2TIMGroupMemberInfoResult) {
                ...
                if (v2TIMGroupMemberInfoResult.getNextSeq() != 0){
                    ...
                    getGroupCustom(mGroupInfo,callBack);
                }
            }
        });
    }
     ...
     * @return
     */
    public static GroupInfo getGroupCustom(final GroupInfo mGroupInfo , final IUIKitCallBack callBack){
        // TODO toString打印
        try {
            List<String> list = new ArrayList<>();
            list.add(mGroupInfo.getId());
...
//                ToastUtil.toastShortMessage("网络异常");
            ...
            final HashMap<String,String> companyKeyHp = new HashMap<>();
                ...
                public void onSuccess(List<V2TIMGroupInfoResult> v2TIMGroupInfoResults) {
                    for (int i = 0; i < v2TIMGroupInfoResults.size(); ++i) {
                        ...
                        if(customInfo.containsKey("company_name")){
                            String  value = new String(customInfo.get("company_name"));
                            companyHp.put(groupID,value);
                        }
                        if(customInfo.containsKey("company_id")){
                            String  value = new String(customInfo.get("company_id"));
                            companyIdHp.put(groupID,value);
                        }
                        if(customInfo.containsKey("is_key_part")){
                            String  value = new String(customInfo.get("is_key_part"));
                            companyKeyHp.put(groupID,value);
                        }
                    }
                    String groupId = mGroupInfo.getId();
                    if(hp.containsKey(groupId)){
                        mGroupInfo.setEvent_group_type(hp.get(groupId));
                        mGroupInfo.setCompany_name(companyHp.get(groupId));
                        mGroupInfo.setCompany_id(companyIdHp.get(groupId));
                        mGroupInfo.setIs_key_part(companyKeyHp.get(groupId));
                    }
