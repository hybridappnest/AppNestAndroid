...
public class ChatLayout extends AbsChatLayout implements GroupChatManagerKit.GroupNotifyHandler {
    ...
    public void setChatInfo(ChatInfo chatInfo) {
        ...
        if (chatInfo != null && chatInfo.getType() == V2TIMConversation.V2TIM_C2C) {
            isGroup = false;
        } else {
            isGroup = true;
        }
        ...
        if (isGroup) {
            ...
            groupInfo.setEvent_group_type(chatInfo.getGroupType());
            groupInfo.setChatName(chatInfo.getChatName());
            mGroupChatManager.setCurrentChatInfo(groupInfo);
            mGroupInfo = groupInfo;
            loadChatMessages(null);
            loadApplyList();
            getTitleBar().getRightIcon().setImageResource(R.drawable.icon_im_chat_title_setting_btn);
    ...
    public void setAlarmTitleInfo(String eventType, int status, int level, String yaylStartTime,
                                  long yaylTime) {
        mNoticeEventLayout.setViewInfo(eventType, status, level, yaylStartTime, yaylTime);
        mNoticeEventLayout.setVisibility(View.VISIBLE);
    }

    public void setAlarmTitleInfoGone() {
        mNoticeEventLayout.setVisibility(View.GONE);
    }

    public void setYJYATitleInfo() {
        mNoticeYJYALayout.setVisibility(View.VISIBLE);
        mNoticeYJYALayout.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                ImHelper.getDBXSendReq().showYJYATasks();
            }
        });
    }
}
