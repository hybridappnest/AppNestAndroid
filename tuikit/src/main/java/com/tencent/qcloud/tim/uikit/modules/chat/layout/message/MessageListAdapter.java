...
public class MessageListAdapter extends RecyclerView.Adapter {
    ...
    TextView selectNum;
    public void setSelectView( TextView view){
        selectNum = view;
    }
    public int getSelectNum(){
        int num = 0;
        for(int i = 0 ;i<mDataSource.size(); i++ ){
            if(mDataSource.get(i).isSelect()){
                num++;
            }
        }
        return  num;
    ...
    public void setSelectNum(int num){
        selectNum.setText("已选择"+num+"条记录");
    }
    @NonNull
    @Override
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        RecyclerView.ViewHolder holder = MessageEmptyHolder.Factory.getInstance(parent, this,mRecycleView, viewType);
        return holder;
    }
    ...
    public void setDataSource(IChatProvider provider,boolean  state) {
        isShowSelect = false;
        hp.clear();
        if (provider == null) {
            mDataSource.clear();
        } else {
            for(MessageInfo messageInfo:mDataSource){
                if(messageInfo.isSelect()){
                    hp.put(messageInfo.getId(),messageInfo.getId());
                }
            }
            mDataSource = provider.getDataSource();
            for(int i =0 ;i<mDataSource.size();i++){
                mDataSource.get(i).setShowSelect(state);
                if(hp.containsKey(mDataSource.get(i).getId())){
                    mDataSource.get(i).setSelect(true);
                }
                if(mDataSource.get(i).isShowSelect()){
                    isShowSelect = true;
                }
            }
    ...
    public void uppDataState(boolean showSelec){
        isShowSelect =showSelec;
        for(int i = 0 ;i<mDataSource.size(); i++ ){
            mDataSource.get(i).setShowSelect(showSelec);
            if(!showSelec){
                mDataSource.get(i).setSelect(showSelec);
            }
        }
