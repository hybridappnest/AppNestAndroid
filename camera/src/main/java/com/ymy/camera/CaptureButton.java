...
public class CaptureButton extends View {
    ...
    public CaptureButton(Context context, int size) {
        ...
        button_state = com.ymy.camera.JCameraView.BUTTON_STATE_BOTH;  //初始化按钮为可录制可拍照
        Logger.i(TAG, "CaptureButtom start");
        duration = 10 * 1000;              //默认最长录制时间为10s
        Logger.i(TAG, "CaptureButtom end");
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(button_size + outside_add_size * 2,
                button_size + outside_add_size * 2);
    }
    ...
    private void startCaptureAnimation(float inside_start) {
        ValueAnimator inside_anim = ValueAnimator.ofFloat(inside_start, inside_start * 0.75f,
                inside_start);
    ...
    //内外圆动画
    private void startRecordAnimation(float outside_start, float outside_end, float inside_start,
                                      float inside_end) {
